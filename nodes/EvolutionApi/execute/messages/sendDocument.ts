	...
	try {
		...
		if (!media.startsWith('http') && !media.startsWith('data:') && !isBase64(media)) {
			throw new NodeApiError(ef.getNode(), {
				...
				description: 'The document must be a valid URL or base64',
			});
		}

		// Helper function to check if it's base64
		...
		const errorData = {
			...
			error: {
				...
				details: error.message.includes('Could not get parameter')
					? 'Verify that all required fields have been filled correctly'
					: error.message,
